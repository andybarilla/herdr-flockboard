...
# Either way, it finishes by linking ~/.local/bin/herdr-flockboard at the
# installed binary, the same entrypoint convention herdr-mirror/reviewr use.
set -u

repo="andybarilla/herdr-flockboard"

script_dir=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
repo_root="$script_dir/.."
out="$repo_root/target/release/flockboard"
base_url="${FLOCKBOARD_BASE_URL:-https://github.com/$repo/releases/download}"
...
build_from_source() {
  ...
  have cargo || {
    echo "flockboard: cargo not found. Install Rust from https://rustup.rs, or re-install at a release tag (--ref v${version:-X.Y.Z}) to get a prebuilt binary." >&2
    exit 1
  }
  cd "$repo_root" || exit 1
  ...
  [ -x "$out" ] || {
    echo "flockboard: cargo build succeeded but $out is missing or not executable (CARGO_TARGET_DIR set?) — unset it and re-run the install." >&2
    exit 1
  }
  link_entrypoint
}
...
fallback() {
  echo "flockboard: $1 — building from source instead." >&2
  rm -rf "${tmpdir:-}"
  build_from_source
  exit
}
...
# ~/.local/bin/herdr-flockboard is the user-facing entrypoint (the convention
...
# counts as Flockboard-managed only when its target's basename is `flockboard`
...
link_entrypoint() {
  ...
  link="$bin_dir/herdr-flockboard"
  mkdir -p "$bin_dir" || {
    echo "flockboard: could not create $bin_dir — create it and re-run the install to get the herdr-flockboard entrypoint." >&2
    exit 1
  }
  if [ -L "$link" ]; then
    target=$(readlink "$link") || exit 1
    case ${target##*/} in
      flockboard) ;; # managed by us — safe to repoint below
      *)
        echo "flockboard: $link already exists and points at $target, which is not a Flockboard binary; not repointing it. Move it aside and re-run the install to create the entrypoint." >&2
        exit 1
        ;;
    esac
  elif [ -e "$link" ]; then
    echo "flockboard: $link already exists and is not a symlink; not overwriting it. Move it aside and re-run the install to create the entrypoint." >&2
    exit 1
  fi
  ln -sfn "$out" "$link" || exit 1
  echo "flockboard: entrypoint $link -> $out"
}
...
asset="flockboard-$version-$triple.tar.gz"
...
tar -xzf "$tmpdir/$asset" -C "$tmpdir" flockboard || fallback "could not unpack $asset"
mkdir -p "$(dirname "$out")"
install -m 755 "$tmpdir/flockboard" "$out" || fallback "could not install the binary to $out"
echo "flockboard: installed prebuilt v$version ($triple), verified SHA-256."
link_entrypoint
