...
# FLOCKBOARD_BASE_URL, under a temp HOME, with a stub binary instead of a
# cargo build. Asserts on ~/.local/bin/herdr-flockboard:
...
#   (e) a dangling managed link (target basename `flockboard`) is repointed
...
printf '#!/bin/sh\necho flockboard stub\n' > "$server/stage/flockboard"
chmod +x "$server/stage/flockboard"
asset="flockboard-$version-$triple.tar.gz"
tar -czf "$server/$asset" -C "$server/stage" flockboard || exit 1
...
home="$tmp/home"
link="$home/.local/bin/herdr-flockboard"
mkdir -p "$(dirname "$link")"

run_install() { # checkout
  env HOME="$home" FLOCKBOARD_BASE_URL="file://$tmp/server" sh "$1/scripts/fetch-or-build.sh" >"$tmp/out.log" 2>&1
}
...
# (e) A dangling managed link (basename `flockboard`, e.g. a deleted checkout)
# is repointed.
rm -f "$link"
ln -s "$tmp/deleted-checkout/target/release/flockboard" "$link"
