...
lock_version=$(awk '/^name = "flockboard"$/ {getline; gsub(/^version = "|"$/, ""); print; exit}' "$root/Cargo.lock")
readme_version=$(sed -n 's|^herdr plugin install andybarilla/herdr-flockboard --ref v\(.*\)$|\1|p' "$root/README.md" | head -n 1)
